...
async def get_data_for_page(page: int, session: RatelimitedSession) -> MangaPage:
    async with session.post("https://graphql.anilist.co", json={"query": query, "variables": {"page": page}}) as resp:
        data = await resp.json()
    return MangaPage(len(data["data"]["Page"]["media"]), data["data"]["Page"]["pageInfo"]["hasNextPage"])

async def run(run_args: ProviderRunArgs) -> int:
    db = run_args.db
    session = run_args.session
    assert db is not None
    cursor = await db.execute("SELECT last_page FROM anilist WHERE category = 'manga'")
    ...
    while not found_last_page:
        ...
        if not page_data.has_next_page:
            ...
            break
        last_page += 1
    await db.execute("INSERT OR REPLACE INTO anilist VALUES ('manga', ?)", (last_page,))
    await db.commit()
    return 50 * (last_page - 1) + count_on_last_page

anilist_manga = Provider(
    command_line_flag_name="anilist-manga",
    run=run,
    needs_db=True,
)